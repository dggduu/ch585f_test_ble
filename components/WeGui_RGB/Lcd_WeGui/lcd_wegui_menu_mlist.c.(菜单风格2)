...
void wegui_show_mList(uint16_t farmes)
{
	...
	#define TOP_SCAPE    (mList_par.list_y_scape) //上方预留空间
	#define BUTTOM_SCAPE (0) //取消下方预留空间 已删除
	...
	uint8_t id_min=(TOP_SCAPE+mList_par.list_y_offset_cur + mList_par.list_y_scape-mList_par.list_font_high)/mList_par.list_y_scape;
	...
	(mList_par.list_y_scape-mList_par.list_font_high) + (SCREEN_HIGH-BUTTOM_SCAPE))/mList_par.list_y_scape;

	//-----显示余下菜单-----
	if(p!=0x00)
	{
		for(id=0;(id+1)<id_min;id++){p=p->nextMenu;if(p==0x00){return ;}};
		while((p!=0x00)&&(temp_y<(SCREEN_HIGH-BUTTOM_SCAPE))&&(id<temp_show_id_max))
		{
	...
	{
		mList_par.cursor_id=1;
	}
	
	//调整菜单位置使得光标完全显示
	curr_target_y0 = (mList_par.list_y_scape)*mList_par.cursor_id-mList_par.list_y_offset_target;
	if(curr_target_y0<TOP_SCAPE)
		mList_par.list_y_offset_target = mList_par.list_y_offset_target + curr_target_y0 - TOP_SCAPE;
	else if(curr_target_y0>(SCREEN_HIGH-BUTTOM_SCAPE)-1-mList_par.list_font_high)
		mList_par.list_y_offset_target = mList_par.list_y_offset_target + curr_target_y0 - ((SCREEN_HIGH-BUTTOM_SCAPE)-1 - mList_par.list_font_high);
	...
	{
		mList_par.scroll_bar_len = (SCREEN_HIGH-BUTTOM_SCAPE-TOP_SCAPE);
		//scroll_bar_pos=0;
	}
	uint8_t id_max=0;
	...
	if(mList_par.scroll_y_offset_save != mList_par.list_y_offset_cur)//界面移动激活显示滚动条
	{
		...
		if(width>0)
		{
			...
			if(temp < (SCREEN_HIGH-BUTTOM_SCAPE))
			{
				//无需滚动条
			}
			else if(temp > (SCREEN_HIGH-BUTTOM_SCAPE))
			{
				temp -= (SCREEN_HIGH-BUTTOM_SCAPE);

				mList_par.scroll_bar_len = (SCREEN_HIGH-BUTTOM_SCAPE-TOP_SCAPE) - ((id_max+1)- (SCREEN_HIGH-BUTTOM_SCAPE-TOP_SCAPE) / mList_par.list_y_scape)*(SCREEN_HIGH-BUTTOM_SCAPE-TOP_SCAPE)/id_max;
				if(mList_par.scroll_bar_len < 10){mList_par.scroll_bar_len = 10;}
				scroll_bar_len_y0 = TOP_SCAPE + (SCREEN_HIGH-BUTTOM_SCAPE-TOP_SCAPE) - mList_par.scroll_bar_len - ((uint32_t)(SCREEN_HIGH-BUTTOM_SCAPE-TOP_SCAPE)-mList_par.scroll_bar_len)*(temp - mList_par.list_y_offset_cur)/temp;
		...
		//---------------------------------------3.上方固定选项-------------------------------------------------
		//底部填充背景色
		lcd_set_driver_mode((lcd_driver_mode_t)0);//设定笔刷颜色(0背景色)
		lcd_fill_box(0,0,SCREEN_WIDTH,TOP_SCAPE);
		
		//横线分割
		lcd_set_driver_mode((lcd_driver_mode_t)COLOUR_MLIST_NORMAL_TEXT);//设定笔刷颜色
		lcd_draw_line(0,TOP_SCAPE,SCREEN_WIDTH,TOP_SCAPE);
		...
		//---------------------------------------4.下方固定选项-------------------------------------------------
			...
			//底部填充背景色
			lcd_set_driver_mode((lcd_driver_mode_t)0);//设定笔刷颜色(0背景色)
			lcd_fill_box(0,(SCREEN_HIGH-BUTTOM_SCAPE),SCREEN_WIDTH,SCREEN_HIGH-1);
			
			//横线分割
			lcd_set_driver_mode((lcd_driver_mode_t)COLOUR_MLIST_NORMAL_TEXT);//设定笔刷颜色
			lcd_draw_line(0,(SCREEN_HIGH-BUTTOM_SCAPE),SCREEN_WIDTH,(SCREEN_HIGH-BUTTOM_SCAPE));
			
			//填充文字
			
			//更改按钮文字
			mlist_button_string = &button_str_normal;//确定 上 下 返回
			//mlist_button_string = &button_str_ctrl;//确定 加 减 返回
