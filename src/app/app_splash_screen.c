...
/* 分步演示任务：每帧只执行一小步（约 40ms 一次，由 portal_progress_draw 驱动）。
 * 禁止在这里使用 Delay_ms 等长阻塞延时——否则会连续阻塞 TMOS_SystemProcess
 * 数秒，直接饿死 BLE 协议栈（连接事件/广播处理全部无法进行，可能导致复位）。
 * 任务结束后调用 Progress_SetSuccess / Progress_SetFailed 通知框架停止。 */
