...
typedef struct {
  const char *title;
  /* 分步任务回调：is_running 期间每帧（每次 page_update）被调用一次，
   * 每次必须快速返回、不得阻塞（禁止 Delay_ms 之类的长延时），
   * 由任务自己推进 task_step，并在结束时调用 Progress_SetSuccess/Failed。 */
  void (*task_callback)(void *ctx);
  progress_status_t status;
  bool is_running;
  uint32_t task_step; /* 任务步进计数（框架在启动时清零） */
  char detail[32];
