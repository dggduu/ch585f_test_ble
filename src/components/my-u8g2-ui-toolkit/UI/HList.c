...
void hlist_init(hlist_t *hl, uint32_t *tick_ptr) {
    if (hl == NULL || tick_ptr == NULL) return;
...
void hlist_add_xbm_item(hlist_t *hl, const char *title, const uint8_t *icon_xbm, const page_component_t *comp, void *ctx) {
    if (hl == NULL || hl->count >= 8) return;
...
void hlist_add_glyph_item(hlist_t *hl, const char *title, uint16_t glyph, const page_component_t *comp, void *ctx) {
    if (hl == NULL || hl->count >= 8) return;
    hl->items[hl->count].title = title;
...
void hlist_add_protected_xbm_item(hlist_t *hl, const char *title, const uint8_t *icon_xbm, const page_component_t *comp, void *ctx, bool guard_flag, char *alert_text) {
    if (hl == NULL || alert_text == NULL || hl->count >= 8) return;
    hl->items[hl->count].title = title;
    hl->items[hl->count].icon_type = ICON_TYPE_XBM;
    hl->items[hl->count].icon_data.xbm = icon_xbm;
    hl->items[hl->count].comp = comp;
...
void hlist_add_protected_glyph_item(hlist_t *hl, const char *title, uint16_t glyph, const page_component_t *comp, void *ctx, bool guard_flag, char *alert_text) {
    if (hl == NULL || alert_text == NULL || hl->count >= 8) return;
    hl->items[hl->count].title = title;
    hl->items[hl->count].icon_type = ICON_TYPE_GLYPH;
    hl->items[hl->count].icon_data.glyph = glyph;
...
void hlist_draw(u8g2_t *u8g2, void *ctx) {
    ...
    // 1. 纯整数动画进度计算 p_256 (0 ~ 256)
    uint32_t anim_ticks = HLIST_ANIM_TICKS ? HLIST_ANIM_TICKS : 1;
    uint32_t elapsed = *hl->main_tick - hl->start_tick;
    int32_t p_256 = (elapsed * 256) / anim_ticks;
    if (p_256 > 256) p_256 = 256;
    if (p_256 < 0)   p_256 = 0;

    // 图标平滑过渡 ease_idx (Q8 定点数)
    int32_t ease_idx_q8 = (hl->from_index << 8) + (hl->to_index - hl->from_index) * p_256;

    int screen_mid = screen_cfg->width / 2;
    int icon_gap = ICON_GAP;

    int total_content_h = ICON_HEIGHT + 6 + screen_cfg->font_height;
    ...
    if (title) {
        ...
        int text_base_y = icon_y + ICON_HEIGHT + 4 + screen_cfg->font_baseline;
        
        if (p_256 < 256) { 
            // 纯整数偏移计算：从 +8px 渐变为 0px
            int y_off = (8 * (256 - p_256)) >> 8; 
