...
typedef void (*portal_draw_cb_t)(u8g2_t *u8g2, int16_t x, int16_t y, uint8_t w,
                                 uint8_t h, void *ctx);
...
#define PORTAL_CTX_BUFFER_SIZE 64
...
typedef struct {
  ...
  // portal 相关
  const portal_component_t *active_portal;
  uint8_t portal_ctx_buffer[PORTAL_CTX_BUFFER_SIZE];
  void *portal_ctx;
